...
test('allows renderer navigation only to app origins', () => {
  ...
  const packagedEntryUrl = 'file:///Applications/Forge3D.app/Contents/Resources/app.asar/dist/index.html';
  assert.equal(isAllowedRendererNavigation(packagedEntryUrl, { packagedEntryUrl }), true);
  assert.equal(isAllowedRendererNavigation('file:///tmp/untrusted.html', { packagedEntryUrl }), false);
  assert.equal(isAllowedRendererNavigation('file:///Applications/Forge3D.app/Contents/Resources/app.asar/dist/other.html', { packagedEntryUrl }), false);
  assert.equal(isAllowedRendererNavigation(packagedEntryUrl), false);
